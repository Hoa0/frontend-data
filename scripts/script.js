 ...
 */
 ...
 async function update(new_data) {
  ...
  .padding(0.2);
SVG_FRUIT.append('g')
  .attr('transform', `translate(0, ${HEIGHT_VALUE})`)
  .call(d3.axisBottom(x))
  .selectAll('text')
    .attr('transform', `translate(-10,0)rotate(-45)`)
    .style('text-anchor', "end")
  ...
  // Render the chart with new data
      const rect = SVG_FRUIT.selectAll('rect').data(new_data, (d) => d.name).join(
  ...
  rect
    .attr('x', d => x(d.name))
    .attr('y', d => VALUE(d.nutritions.carbohydrates))
    .attr('width', x.bandwidth())
    .attr('height', d => HEIGHT_VALUE - VALUE(d.nutritions.carbohydrates))
    .style('fill', '#ffcc00')
